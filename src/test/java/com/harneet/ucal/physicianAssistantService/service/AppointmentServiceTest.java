...
class AppointmentServiceTest {
    ...
    void getAllAppointmentsReturnsListOfAppointments() {
        ...
        // Update mock setup to match the actual method call
        when(appointmentRepository.findAll("1")).thenReturn(Arrays.asList(appointment1, appointment2));

        List<Appointment> appointments = appointmentService.getAllAppointments("1");

        assertEquals(2, appointments.size());
        verify(appointmentRepository, times(1)).findAll("1");
    }
